"""
normalize_linkedin.py — P95.AI Lead Engine
Normalizes LinkedIn pass CSV files,
deduplicates against existing leads, and writes:
  data/raw/linkedin_normalized.csv

Usage: python scripts/normalize_linkedin.py
"""
...
SOURCES = [
    Path("data/raw/linkedin_pass1_us (1).csv"),
    Path("data/raw/linkedin_pass2_eu.csv"),
    Path("data/raw/linkedin_pass3_india.csv"),
]

EXISTING_NORMALIZED = [
    Path("data/raw/apollo_normalized.csv"),
    Path("data/raw/seeds_normalized.csv"),
    Path("data/raw/engineer_normalized.csv"),
    Path("data/raw/aayush_normalized.csv"),
]

OUTPUT = Path("data/raw/linkedin_normalized.csv")
...
def map_emp_range(count_str: str, current_range: str) -> str:
    if current_range in VALID_EMP_RANGE:
...
def norm_source(v: str) -> str:
    ...
    return v if v in VALID_SOURCES else "linkedin"
...
def main():
    ...
    out_rows   = []
    stats      = {"loaded":0,"competitors":0,"written":0}
    ...
    for src_file in SOURCES:
        ...
        for _, raw in df.iterrows():
            ...
            row["employee_range"] = map_emp_range(row.get("employee_count",""), row.get("employee_range", ""))
